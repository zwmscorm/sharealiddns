...
aliddns_account_conf="$aliddns_root/conf/account.conf"
aliddns_mail_conf="$aliddns_root/conf/emil.conf"
aliddns_odhcp6c="$aliddns_root/var/odhcp6c_cmd"
aliddns_log="$aliddns_root/log/log.txt"
aliddns_msg="$aliddns_root/log/msg.txt"
...
#======================================================================================
#======================================================================================
logs "$0 $1 $2 $3 $4" "$LS" "rb"
#======================================================================================
#0:close debug
#1:get_Router_options
#2:get_aliddns_options
#3:get_aliddns_account
#6:aliddns_domain_api and get_Record
ndebug=0
#======================================================================================
isGEbetween 0 "$ARGS4" 6 && ndebug="$ARGS4"
#======================================================================================
islog=0
isipv4_domain=0
isipv6_domain=0
...
ipv6_client="/tmp/ipv6_active_client"
#======================================================================================
routerddns_no=1
routerddns_name=""
routerddns_domain=""
...
#======================================================================================
get_other_conf(){
    if [ -f "$aliddns_account_conf" ];then
		u4=$(cat $aliddns_account_conf  | grep -w 'u4' | sed "s/u4=//g")
		u6=$(cat $aliddns_account_conf  | grep -w 'u6' | sed "s/u6=//g")
		dns=$(cat $aliddns_account_conf | grep -w 'dns' | sed "s/dns=//g")
    fi
	if isNotEmpty "$u4" && isNotEmpty "$u6" && isNotEmpty "$dns";then
        return 0
	else
	    logs "u4, u6 or dns error, please check." "" "rb" "e"
	    return 1
	fi	
}
#======================================================================================
get_Router_options(){
	local iscontinue=0;local s0="$aliddns_account_conf";local s1="aliddns_domain"
	local s3="isipv4_domain";local s4="isipv6_domain"
	local s5="pppoe_ifname";local s6="cron_Time";local s7="cron_Time_type";local s8="islog"
	local t1=0;local t2=0;local t3=0;local v1="";local n1=0
	islog=0;isipv6=0;aliddns_TotalCount=0
	
	v1=$(cat $s0    | grep -w $s1 | RMSTRING "$s1=")
	n1=$(echo "$v1" | awk 'END{print NR}')
	isNotEmpty "$v1" && isipv6=1                 
	isNotEmpty "$n1" && aliddns_TotalCount="$n1" 
	
	isipv4_domain=$(cat $s0  | grep -w $s3 | RMSTRING "$s3=" | TRIMALL)
    isipv6_domain=$(cat $s0  | grep -w $s4 | RMSTRING "$s4=" | TRIMALL)
	pppoe_ifname=$(cat $s0   | grep -w $s5 | RMSTRING "$s5=" | TRIMALL)
	cron_Time=$(cat $s0      | grep -w $s6 | RMSTRING "$s6=" | TRIMALL)
	cron_Time_type=$(cat $s0 | grep -w $s7 | RMSTRING "$s7=" | TRIMALL)
	islog=$(cat $s0 | grep -w $s8 | RMSTRING "$s8=" | TRIMALL)

	if iseq "$ndebug" 1;then
	    logs "*********************************************************************"
	    logs "isipv6=$isipv6"                         "" "yb"
	    ...
	    logs "aliddns_TotalCount=$aliddns_TotalCount" "" "yb"
		logs "*********************************************************************"
	fi
	
	if isge "$aliddns_TotalCount" 0;then
	    iscontinue=1
	else
	    logs "$s0 has a formatted error, please check." "" "rb" "e"
        return 1		 
	fi
	if iseq "$pppoe_ifname" "wan0" || iseq "$pppoe_ifname" "wan1" || iseq "$pppoe_ifname" "auto" || iseq "$pppoe_ifname" "any";then 
	    iscontinue=1
	else
	    logs "pppoe_ifname has a wan0|wan1|auto|any, please check." "" "rb" "e"
		return 1
	fi
	if iseq "$islog" 0 || iseq "$islog" 1;then 
	    iscontinue=1
	else
	    logs "islog has a 0 or 1, please check." "" "rb" "e"
		return 1
	fi
	if iseq "$isipv4_domain" 0 || iseq "$isipv4_domain" 1;then
	    iscontinue=1
	else
	    logs "isipv4_domain has a 0 or 1, please check." "" "rb" "e"
		return 1
	fi
	if iseq "$isipv6_domain" 0 || iseq "$isipv6_domain" 1;then
	    iscontinue=1
	else
	    logs "isipv6_domain has a 0 or 1, please check." "" "rb" "e"
		return 1
	fi
	if iseq "$cron_Time_type" "hour" || iseq "$cron_Time_type" "min";then
	    iscontinue=1
	else
	    logs "cron_Time_type has a hour or min, please check." "" "rb" "e"
		return 1
	fi
	if iseq "$cron_Time_type" "min";then
	    if isge "$cron_Time" 5 && isle "$cron_Time" 59;then
	        ...
	        logs "cron_Time has a 5~59, please check." "" "rb" "e"
            return 1
        fi	
	    ...
	    if isge "$cron_Time" 1 && isle "$cron_Time" 23;then
	        ...
	        logs "cron_Time has a 1~23, please check." "" "rb" "e"
            return 1
        fi		
	fi	
	return 0
...
#======================================================================================
get_aliddns_options(){
    local s0="$aliddns_account_conf";local s1="aliddns_AccessKeyId";local s2="aliddns_AccessKeySecret"
	local s3="routerddns_no";local s4="aliddns_domain";local s5="aliddns_name";local iscontinue=0
	aliddns_AccessKeyId=$(cat $s0     | grep -w $s1 | TRIMALL | RMSTRING "$s1=" | TRIMALL)
	aliddns_AccessKeySecret=$(cat $s0 | grep -w $s2 | TRIMALL | RMSTRING "$s2=" | TRIMALL)
	routerddns_no=$(cat $s0           | grep -w $s3 | TRIMALL | RMSTRING "$s3=" | TRIMALL)
	...
	if isNotEmpty "$aliddns_AccessKeyId" && isNotEmpty "$aliddns_AccessKeySecret";then
	    iscontinue=1
    else
		logs "aliddns_AccessKeyId or aliddns_AccessKeySecret has a formatted error, please check." "" "rb" "e"
		return 1
	fi	
	if check_number "$routerddns_no";then
	    if isle "$routerddns_no" 0 || isgt "$routerddns_no" "$aliddns_TotalCount";then
	        logs "Router show ddns routerddns_no has change value is 1." "" "rb" "e"
	        routerddns_no=1
			set_routerddns_no 1
	    fi
	else
	    logs "routerddns_no has a formatted error, please check." "" "rb" "e"
		return 0
	fi
	routerddns_domain=$(cat $s0 | grep -w $s4 | TRIMALL | RMSTRING "$s4=" | awk "NR==$routerddns_no{print}" | TRIMALL)
	routerddns_name=$(cat $s0   | grep -w $s5 | TRIMALL | RMSTRING "$s5=" | awk "NR==$routerddns_no{print}" | TRIMALL)
	if iseq "$ndebug" 2;then
	    logs "*********************************************************************"
...
#======================================================================================
get_aliddns_account(){
    local c="$1";local s0="$aliddns_account_conf";local s1="aliddns_domain";local s2="aliddns_name"
	...
	if isEmpty "$aliddns_domain_list" && isEmpty "$aliddns_name_list" && isEmpty "$aliddns_dns_list" && isEmpty "$aliddns_ttl_list" && isEmpty "$aliddns_type_list";then
	    aliddns_domain_list=$(cat $s0 | grep -w $s1  | TRIMALL | RMSTRING "$s1=")
	    aliddns_name_list=$(cat $s0 | grep -w $s2    | TRIMALL | RMSTRING "$s2=")
	    aliddns_ttl_list=$(cat $s0 | grep -w $s3     | TRIMALL | RMSTRING "$s3=")
	    aliddns_type_list=$(cat $s0 | grep -w $s4    | TRIMALL | RMSTRING "$s4=")
	    aliddns_lan_mac_list=$(cat $s0 | grep -w $s5 | TRIMALL | RMSTRING "$s5=")
	fi
	aliddns_domain=$(echo -e "$aliddns_domain_list"   | awk "NR==$c{print}" | TRIMALL)
	aliddns_name=$(echo -e "$aliddns_name_list"       | awk "NR==$c{print}" | TRIMALL)
	aliddns_ttl=$(echo -e "$aliddns_ttl_list"         | awk "NR==$c{print}" | TRIMALL)
	aliddns_type=$(echo -e "$aliddns_type_list"       | awk "NR==$c{print}" | TRIMALL)
	aliddns_lan_mac=$(echo -e "$aliddns_lan_mac_list" | awk "NR==$c{print}" | TRIMALL | set_lowercase)
	iseq "$aliddns_type" "A" && aliddns_lan_mac=""
	if iseq "$ndebug" 3;then
	    logs "*********************************************************************"
	    logs "aliddns_domain=[$aliddns_domain]"   "" "yb"
 	    logs "aliddns_name=[$aliddns_name]"       "" "yb"
	    logs "aliddns_ttl=[$aliddns_ttl]"         "" "yb"
	    logs "aliddns_type=[$aliddns_type]"       "" "yb"
		logs "aliddns_lan_mac=[$aliddns_lan_mac]" "" "yb"
		logs "*********************************************************************"
	fi
    if isNotEmpty "$aliddns_lan_mac" && ! valid_mac "$aliddns_lan_mac";then
	    logs "aliddns_lan_mac has a formatted error, please check." "" "rb" "e"
		return 1
	fi
    if ! check_number "$aliddns_ttl";then
	    logs "aliddns_ttl has a formatted error, please check." "" "rb" "e"
		return 1
	fi
	if isEmpty "$aliddns_name";then
	    logs "aliddns_name has a formatted error, please check." "" "rb" "e"
		return 1
	fi
	if isEmpty "$aliddns_domain";then
	    logs "aliddns_domain has a formatted error, please check." "" "rb" "e"
		return 1
	fi
	if iseq "$aliddns_type" "A" || iseq "$aliddns_type" "AAAA";then
	    return 0
	else
        logs "aliddns_type has a formatted error, please check." "" "rb" "e"
		return 1    
	fi
}
#======================================================================================
set_routerddns_no(){
    local n="$1";local s0="$aliddns_account_conf";local s1="routerddns_no"
	local v1=$(cat $s0 | grep -w $s1);local v2="$s1=$n"
	do_sed "$v1" "$v2" "$s0"  
}
#======================================================================================
get_Record_list() {
...
aliddns_domain_api(){
	...
	if iseq "$ndebug" 6 && iseq "$num_getRecord" 1;then
	    echo "$QU"
	else
	    echo "$($QU)"
	fi
...
do_check(){
	...
	if iseq "$aliddns_type" "A" || iseq "$aliddns_type" "AAAA";then
	    ...
	    logs "$Count:[$aliddns_type]-[$aliddns_name.$aliddns_domain]type error, Please check $aliddns_account_conf." "" "rb" "e"
		logs "$Count:failed..." "" "rb" "e"
	    return 1 
	fi
}
...
#======================================================================================
goto_ping_wake(){
    local ml="$1";local s0="$aliddns_account_conf";local s1="aliddns_lan_mac"
...
goto_check(){
	...
	if isEmpty "$1" || iseq "$1" 0 || ! isNumber "$1";then
	    ...
	    until [ "$Count" -gt "$aliddns_TotalCount" ];do  
		    logs "$Count:going..." "" "rb"	
		    get_aliddns_account "$Count" && do_check 
			Count=$(sadd $Count 1)	
	    done
        return 0  
...
goto_start(){
	...
	if isEmpty "$1" || iseq "$1" 0 || ! isNumber "$1";then
	    ...
	    until [ "$Count" -gt "$aliddns_TotalCount" ];do  
		    logs "$Count:going..." "" "rb"	
		    get_aliddns_account "$Count" && do_check && do_start
			Count=$(sadd $Count 1)	
	    done
		return 0
	elif isNumber "$1";then
...
goto_checkwanip(){
    ...
    if isNumber "$1";then
	    if isgt "$1" 0 && isle "$1" "$aliddns_TotalCount";then
	        Count="$1"
			logs "$Count:going..." "" "rb"	
	        get_aliddns_account "$Count" && do_checkwanip
	        return 0
		fi
	else
	    Count=1
...
goto_status(){
	...
	if isgt "$1" 0 && isle "$1" "$aliddns_TotalCount";then
	    Count="$1"
		logs "$Count:going..." "" "rb"
	    get_aliddns_account "$Count" && get_Record && do_Record_Only_check && do_status "$2"
		return 0
	elif iseq "$1" 0;then		
	    Count=1
	    until [ "$Count" -gt "$aliddns_TotalCount" ];do  
...
goto_remove(){
    ...
    if iseq "$1" 0;then
	    ...
	    until [ "$Count" -gt "$aliddns_TotalCount" ];do  
			logs "$Count:going..." "" "rb"	
		    if get_aliddns_account "$Count";then
				if do_remove;then
				    do_aliddns_log "$actions" "success"
                    set_success 1 "$actions" "success"					
			    else
	...
	if get_wan_ipv46;then
		if iseq "$aliddns_type" "AAAA";then
	        if isNotEmpty "$aliddns_lan_mac";then
			    ...
			    if isNotEmpty "$lan_ipv6_IP";then
			        log="$aliddns_name.$aliddns_domain $lan_ipv6_IP $aliddns_lan_mac $lan_ipv4_IP"
					if [ ! -f "$ipv6_client" ];then
	                    echo "$log" >> "$ipv6_client" 
	                else
	                    echo "$log" > "$ipv6_client" 
	                fi
			    fi
...
#======================================================================================
do_speed_dns_bak(){
    local u="$1";local v="/tmp/dnsnslookup.tmp";local t=0;local s="";local r=1
	local SORT=`which sort`;local NSLOOKUP=`which nslookup`;local TIME=`which time`
	if [ -z "$SORT" -a -z "$NSLOOKUP" -a -z "$TIME" ];then
        ...
        for p in $dns;do
		    ...
		    t="$(cat "$v" | grep -w 'real' | sed 's/real//g' | sed 's/[[:space:]]//g')"
		    if [ "$r" -eq 0 ] && [ -n "$t" ] && echo -e "$t" | grep -q '[0-9].\+$' 2>/dev/null;then		
			    if [ -z "$s" ];then
				    s="${t} $p"
		        else
				    s="${s}=${t} $p"
...
#======================================================================================
service_check(){
    local i=1;local j=30;local n=0;local r=1  
    ...
    until [ "$i" -gt "$j" ];do
		...
		logs "$i/$j:Ensure that all services are run finish." "" "y" "i"
		isge "$i" "$n" && r=0 && break
	    sleep 1
		i=$(sadd $i 1)	
    done
...
#======================================================================================
do_init(){
    if [ "$2" != "start" -a "$2" != "stop" -a "$2" != "restart" -a "$2" != "check" -a "$2" != "update" -a "$2" != "again" -a "$2" != "add" -a "$2" != "removeall" -a "$2" != "remove" -a "$2" != "status" -a "$2" != "monitor" -a "$2" != "checkwanip" -a "$2" != "showlog" -a "$2" != "kill" -a "$2" != "client" ];then
        logs "Usage: $1 {start|stop|restart|check|update|again|add|removeall|remove|status|monitor|checkwanip|showlog|kill|client}" "" "yb" "w" >&2
        return 1
    fi
	
	if isNotEmpty "$(echo $scripts_sh | grep -o 'mnt')";then
        local p=$(echo $scripts_sh | sed 's~/tmp~~g' | sed 's~/mnt/~~g' | awk -F '/' '{print $1}')
        if echo -e "$p" | grep -q '^[a-zA-Z0-9]\+$' && isge $(str_total "$p") 5;then
	        p=""
        else
	        logs "低级错误：USB分区卷标名必须设置为英文或数字，而且总数必须超过4位！" "" "ra" "e"	
			logs "比如：u盘、我的U盘、sda1、sdc1、usb1等都是错误的！" "" "ra" "e"
			logs "比如：mydata、myusb2、usbusb2、USBdisk等都是正确的！" "" "ra" "e"
            exit 0			
        fi
    fi
...
#======================================================================================
do_begin(){
    if service_check && do_init "$1" "$2" "$3" && logs "" "$LS" && get_other_conf && get_Router_options && get_aliddns_options && set_ipv6_config && get_wan_info && get_externalIP;then   
		goto_ping_wake
