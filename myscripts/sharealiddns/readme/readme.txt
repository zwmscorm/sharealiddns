...
======================================================================================
重要提示：    
　  如果你要进行IPV6域名解析，首先确认运营商已经推送IPV6，其次确保路由器固件支持IPV6并已开启IPV6功能，
检查是否已经成功获得IPV6 IP，否则必须关闭IPV6，如何关闭IPV6？可以在路由器界面上进行设置，比如
pandorabox、lede/openwrt固件的路由器需在网络--接口--WAN--高级设置--Obtain IPv6-Address Disabled--禁用，
或者打开/etc/config/network文件，按下面设置：  
option ipv6 '0'  
重启，使设置生效。
======================================================================================
私有概念定义：  
...
配置文件aliddns.conf中的isipv4_domain和isipv6_domain参数设置：
    isipv4_domain设置为1，允许脚本执行IPV4域名解析，设置为0，不允许脚本执行IPV4域名解析。
    isipv6_domain设置为1，允许脚本执行IPV6域名解析，设置为0，不允许脚本执行IPV6域名解析。
    注意这里指的是脚本，不是固件。
======================================================================================
